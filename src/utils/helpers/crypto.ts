...
export function encrypt(value: string | number): string {
  return encodeURIComponent(CryptoJS.AES.encrypt(String(value), DEFAULT_KEY).toString());
}
...
export function decrypt(encryptedValue: string): string {
  const decrypted = CryptoJS.AES.decrypt(decodeURIComponent(encryptedValue), DEFAULT_KEY);
